...
def predict(modelpath, filepath, pathsave='./output'):
    ...
    with torch.no_grad():
        ...
        # label_values = mesh3.celldata['Label'].tolist()
        label_values = refine_labels.tolist()
        vertices = mesh.points()
        faces = mesh.faces()
        # vertices_list = vertices.astype(np.int32).tolist()
        # faces_list = faces
        # vertices_list = np.array(vertices)
        # faces_list = np.array(faces)
        # cell_data = {"label_values": label_values, "vertices": vertices.tolist(), "faces": faces.tolist()}
        data = {"label_values": label_values}
        with open(os.path.join(pathsave, '{}_refined.json'.format('out')), 'w') as outfile:
            json.dump(data, outfile)


        # cell_data = {}
        # for i in range(mesh3.NCells()):
        #     cell_data[int(i)] = int(mesh3.celldata['Label'][i])
        #
        ...
        #     output_str += '{}   {}\n'.format(key, value)
        # with open(os.path.join(pathsave, '{}_downsampling_refined.json'.format('out')), 'w') as outfile:
        #     json.dump(cell_data, outfile)