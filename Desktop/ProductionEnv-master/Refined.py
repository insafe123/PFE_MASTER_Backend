...
def predict(modelpath, filepath, pathsave='./output'):
    ...
    with torch.no_grad():
        ...
        # #########################################Colorisation#######################################################
        # # create a dictionary that maps class labels to colors
        # color_map = {0: 'red', 1: 'green', 2: 'blue', 3: 'yellow', 4: 'magenta', 5: 'cyan', 6: 'gray', 7: 'orange',
        #              8: 'brown', 9: 'purple', 10: 'pink', 11: 'olive', 12: 'navy', 13: 'teal', 14: 'lime'}
        #
        # # load the mesh
        # mesh = vedo.load(filepath)
        #
        # # get the predicted labels for each cell
        # predicted_labels = []
        # for label in fine_labels:
        #     if isinstance(label, np.ndarray):
        #         predicted_labels.append(tuple(label))
        #     else:
        #         predicted_labels.append(label)
        #
        # predicted_labels = fine_labels.astype(int).tolist()
        # predicted_labels = [tuple(label) if isinstance(label, np.ndarray) else label for label in
        #                     predicted_labels]  # convert ndarray to tuple
        #
        # # create a color map for the mesh using the dictionary and the predicted labels
        # colors = [color_map[label] for label in predicted_labels]
        # colormap = vedo.ColorMap(colors)
        #
        # # assign the colors to the cells of the mesh
        # mesh.color(colormap, mode='cell')
        #
        # # get the colored object
        # vedo.write(mesh, os.path.join(pathsave, '{}_downsampling_colored.vtp'.format('out')))
